...
from channels.schemas import Channel, NewChannel
...
async def add_channel(
    ...
    if authorized:
        ...
        try:
            os.makedirs(
                f'/Users/matvey/AutoPostingChannels/{data["name"]}', exist_ok=True
            )

            path_to_source_dir = (
                f'/Users/matvey/AutoPostingChannels/{data["name"]}/source/'
            )
            path_to_except_dir = (
                f'/Users/matvey/AutoPostingChannels/{data["name"]}/except/'
            )
...
async def update_channel(
    channel: Channel, authorized: bool = Depends(authenticate_user)
):
    if authorized:
        try:
            data = channel.model_dump()
